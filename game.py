...
class game:
    ...
    def play_turn(self, player, piece, new_pos):
        ...
        if type(piece).__name__ == "Pawn":
            if piece.color == "white" and new_pos[1] == 7:
                nouvelle_piece = Queen("white", new_pos)
                self.board.board[new_pos[0]][new_pos[1]] = nouvelle_piece
                self.wplayer1.pieces.remove(piece)
                self.wplayer1.pieces.append(nouvelle_piece)
            elif piece.color == "black" and new_pos[1] == 0:
                nouvelle_piece = Queen("black", new_pos)
                self.board.board[new_pos[0]][new_pos[1]] = nouvelle_piece
                self.bplayer2.pieces.remove(piece)
                self.bplayer2.pieces.append(nouvelle_piece)
